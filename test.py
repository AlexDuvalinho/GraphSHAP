...
class GraphConv(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        y = torch.matmul(torch.tensor(adj).double(), x.double())
...
class GcnEncoderGraph(nn.Module):
    ...
    def gcn_forward(
        ...
        adj_att_tensor = torch.stack(adj_att_all, dim=2)
...
class GcnEncoderNode(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes=None, **kwargs):
        ...
        pred = self.pred_model(self.embedding_tensor.float())
        return pred, adj_att
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        if self.linkpred:
            max_num_nodes = adj.size()[1]
            pred_adj0 = self.assign_tensor @ torch.transpose(
                self.assign_tensor, 1, 2)
            tmp = pred_adj0
            pred_adj = pred_adj0
            ...
            if batch_num_nodes is None:
                ...
                embedding_mask = self.construct_mask(
                    max_num_nodes, batch_num_nodes)
                adj_mask = embedding_mask @ torch.transpose(
                    embedding_mask, 1, 2)
...
model(data.x.double(), torch.tensor(data.edge_index).double())
